...
def find_matching_videos_filtered(
    keys,
    video_dir,
    as_dict=False,
    recursive=True,
    ...
    for key in keys:
        ...
        if not matches:
            print(f"Skipping key {key} because no matching video found.")
            continue
        longest_match = sorted(matches, key=lambda v: len(v))[-1]
        video_paths[key] = videos_to_paths[longest_match]

    if as_dict:
        return video_paths
    else:
        return [video_paths[k] for k in keys if k in video_paths]
