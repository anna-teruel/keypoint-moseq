...
def interactive_group_setting(project_dir, model_name, num_factors = 1):
    """Start the interactive group setting widget for multiple factors."""
    ...
    # Ensure only the specified number of factors exist and are initialized
    expected_factors = [f"group{i}" for i in range(1, num_factors + 1)]
    for factor in expected_factors:
        if factor not in summary_data.columns:
            summary_data[factor] = "default"  # Initialize missing columns with 'default'
    
    # Remove any extra group columns that are not needed
    existing_factors = [col for col in summary_data.columns if col.startswith("group") and col not in expected_factors]
    summary_data.drop(columns=existing_factors, inplace=True, errors='ignore')
    
    titles = {"name": "recording name"}
    editors = {"name": None}

    for factor in expected_factors:
        titles[factor] = factor
        editors[factor] = {
            "type": "textarea",
            "elementAttributes": {
                "maxlength": "100",
    ...
    button = pn.widgets.Button(name="Save info", button_type="primary")

    def save_index(event):
        summary_data.to_csv(index_filepath, index=False)

    button.on_click(save_index)
    return pn.Row(summary_table, pn.Column(button))



# def compute_moseq_df(project_dir, model_name, *, fps=30, smooth_heading=True, num_factors=1):
...
#     results_dict = load_results(project_dir, model_name)
#     index_filepath = os.path.join(project_dir, "index.csv")
#     index_data = pd.read_csv(index_filepath, index_col=False) if os.path.exists(index_filepath) else None
    
#     recording_name, centroid, velocity, heading, angular_velocity, syllables, frame_index = [], [], [], [], [], [], []
#     factor_groups = {f"group{i}": [] for i in range(1, num_factors + 1)}
    
#     # Ensure all expected group columns exist
#     if index_data is not None:
#         for factor in factor_groups.keys():
#             if factor not in index_data.columns:
#                 index_data[factor] = "default"  # Fill missing columns with default value
    
#     for k, v in results_dict.items():
#         n_frame = v["centroid"].shape[0]
#         recording_name.append([str(k)] * n_frame)
#         centroid.append(v["centroid"])
#         velocity.append(np.concatenate(([0], np.sqrt(np.square(np.diff(v["centroid"], axis=0)).sum(axis=1)) * fps)))

#         if index_data is not None:
#             for factor in factor_groups.keys():
#                 if k in index_data["name"].values:
#                     factor_value = index_data.loc[index_data["name"] == k, factor].values[0]
#                 else:
#                     factor_value = "default"
#                 factor_groups[factor].append([factor_value] * n_frame)
#         else:
#             for factor in factor_groups.keys():
#                 factor_groups[factor].append(["default"] * n_frame)

#         frame_index.append(np.arange(n_frame))
#         heading.append(filter_angle(v["heading"]) if smooth_heading else v["heading"])
#         angular_velocity.append(np.concatenate(([0], np.diff(filter_angle(v["heading"], size=3, method="gaussian")) * fps)))
#         syllables.append(v["syllable"])

#     moseq_df = pd.DataFrame(np.concatenate(recording_name), columns=["name"])
#     column_names = ["centroid_x", "centroid_y"] if centroid[0].shape[1] == 2 else ["centroid_x", "centroid_y", "centroid_z"]
#     moseq_df = pd.concat([moseq_df, pd.DataFrame(np.concatenate(centroid), columns=column_names)], axis=1)
...
#     for factor in factor_groups.keys():
#         moseq_df[factor] = np.concatenate(factor_groups[factor])
    
#     onset = np.full(moseq_df.shape[0], False)
#     onset[np.concatenate(([0], np.where(np.diff(moseq_df["syllable"]) != 0)[0] + 1))] = True
...
def  run_multifactor_kruskal(
    ...
    """Run Kruskal-Wallis test for multiple factors and Dunn's post hoc test with permutation-based correction.
    
    Returns:
    - df_k_real: DataFrame with KW test results per syllable.
    - dunn_results_df: DataFrame with Dunn’s test results and corrected p-values.
    - intersect_sig_syllables: Dictionary of significant syllables per group comparison.
    """
    rnd = np.random.RandomState(seed=seed)
    ...
    grouped_data = (
        stats_df.pivot_table(
            index=["group1", "group2", "name"], columns="syllable", values=statistic
        )
        .replace(np.nan, 0)
        .reset_index()
    )
    ...
    # Compute KW constants
    vc = grouped_data.groupby(["group1", "group2"]).size()
    n_per_group = vc.values
    group_names = vc.index
    cum_group_idx = np.insert(np.cumsum(n_per_group), 0, 0)
    num_groups = len(group_names)
    ...
    # Get all syllable usage data
    df_only_stats = grouped_data.drop(["group1", "group2", "name"], axis=1)
    syllable_data = df_only_stats.values
...
def run_multifactor_anova(
    stats_df,
    statistic="frequency",
    num_factors=2,
    thresh=0.05,
):
    """Run two-way ANOVA to compare parametric results with Kruskal-Wallis, returning significant results."""
    
    if num_factors < 2:
        raise ValueError("ANOVA requires at least two factors.")
    
    grouping_cols = [col for col in ["group1", "group2"] if col in stats_df.columns][:num_factors]
    syllables = stats_df["syllable"].unique()
    sig_results = {}
    
    for syllable in syllables:
        syll_df = stats_df[stats_df["syllable"] == syllable]
        formula = f"{statistic} ~ {' + '.join(grouping_cols)} + {' * '.join(grouping_cols)}"
        model = smf.ols(formula, data=syll_df).fit()
        anova_table = sm.stats.anova_lm(model, typ=2)
        
        # Store p-values
        sig_results[syllable] = anova_table["PR(>F)"].to_dict()
...
def plot_syll_stats_with_sem_grouped(
    stats_df,
    project_dir,
    model_name,
    save_dir=None,
    ...
    """Plot syllable statistics with grouped categories (based on syll_info.csv), including statistical testing,
    generating multiple plots based on selected grouping factors.
    ...
    num_factors : int, optional
        Number of grouping factors, by default 1.
    group_to_plot : str, optional
        The group factor to generate multiple plots for, default is 'group2'.
    groups : list, optional
        The list of groups to plot, by default None.
    ctrl_group : str, optional
        The control group, by default None.
    ...
    legends : list
        List of matplotlib legend objects.
    """
    
    # Construct the syllable info path
    syll_info_path = os.path.join(project_dir, model_name, "syll_info.csv")
    
    # Load syllable category mappings
    syll_info_df = pd.read_csv(syll_info_path)  # Load syllable info CSV
    syll_dict = dict(zip(syll_info_df['syllable'], syll_info_df['label']))  # Map syllables to behavior labels
    
    # Replace syllable numbers with behavior categories
    stats_df["syllable"] = stats_df["syllable"].map(syll_dict)
    
    # Ensure 'name' column exists before grouping
    if "name" not in stats_df.columns:
        raise KeyError("The 'name' column is missing. Ensure it's present before running statistical tests.")
    
    # Aggregate statistics by behavior category, keeping 'name'
    stats_df = stats_df.groupby([group_to_plot, "group1", "name", "syllable"])[stat].agg(["mean", "sem"]).reset_index()
    stats_df.columns = [group_to_plot, "group1", "name", "syllable", stat, "sem"]  # Flatten column names
    
    unique_levels = stats_df[group_to_plot].unique()
    figs = []
    legends = []
    
    for level in unique_levels:
        subset_df = stats_df[stats_df[group_to_plot] == level]
        
        # Run Kruskal-Wallis and Dunn's test for significance
        sig_sylls = None
        if plot_sig and len(subset_df["group1"].unique()) > 1:
            _, _, sig_pairs = run_kruskal(subset_df, statistic=stat, thresh=thresh)
            sig_sylls = sig_pairs if sig_pairs else None
        
        xlabel = f"Syllables sorted by {stat}"
        if order == "diff":
            xlabel += " difference"
        ordering, plot_groups, colors, figsize = _validate_and_order_syll_stats_params(
            subset_df,
            stat=stat,
            order=order,
            groups=groups,
        ...
        fig, ax = plt.subplots(1, 1, figsize=figsize)
        hue = "group1" if "group1" in stats_df.columns else "group"
        ...
        if sig_sylls is not None:
            init_y = -0.05
            if isinstance(sig_sylls, dict):
                for key, syll_list in sig_sylls.items():
                    markings = [np.where(ordering == s)[0] for s in syll_list if s in ordering]
                    if markings:
                        markings = np.concatenate(markings)
                        plt.scatter(markings, [init_y] * len(markings), color="r", marker="*")
                        plt.text(
                            plt.xlim()[1],
                            init_y,
                            f"{key[0]} vs. {key[1]} - Total {len(syll_list)} S.S.",
                        )
                        init_y += -0.05
            else:
                markings = [np.where(ordering == s)[0] for s in sig_sylls if s in ordering]
                if markings:
                    markings = np.concatenate(markings)
                    plt.scatter(markings, [-0.05] * len(markings), color="r", marker="*")
        ...
        legend = ax.legend(handles=handles, frameon=False, bbox_to_anchor=(1, 1))
        plt.xlabel(xlabel, fontsize=12)
        plt.title(f"Syllable Statistics for {group_to_plot} = {level}")
        sns.despine()

        plot_name = f"{stat}_{order}_grouped_stats_{group_to_plot}_{level}"
        save_analysis_figure(fig, plot_name, project_dir, model_name, save_dir)
...
def plot_syll_multifactor(
    stats_df,
    project_dir,
    model_name,
    group_plot="group2",  # Defines the number of plots (e.g., days)
    group_diff="group1",  # Defines the differences inside each plot (e.g., conditions)
    save_dir=None,
    stat="frequency",
    order="stat",
    colors=None,
    figsize=(10, 5),
    sig_sylls=None,  # Dictionary of significant syllables from Kruskal-Wallis
):
    """Plot syllable frequency across different levels of `group2`, comparing `group1` within each plot.
    Excludes individual data points and displays statistical significance markers.
    """
    
    if group_plot not in stats_df.columns or group_diff not in stats_df.columns:
        raise KeyError(f"Both '{group_plot}' and '{group_diff}' must be present in stats_df.")
    
    unique_days = stats_df[group_plot].unique()
    figs = []
    
    for day in unique_days:
        subset_df = stats_df[stats_df[group_plot] == day]
        
        fig, ax = plt.subplots(figsize=figsize)
        
        # Pointplot to show means with SEM (join=False)
        sns.pointplot(
            data=subset_df,
            x="syllable",
            y=stat,
            hue=group_diff,
            dodge=True,
            join=False,  # Disabled joining of points
            markers="o",
            errorbar=("se"),
            palette=colors,
            ax=ax,
        ...
        # Mark statistically significant syllables
        if sig_sylls and day in sig_sylls:
            for syllable, comparisons in sig_sylls[day].items():
                if syllable in subset_df["syllable"].unique():
                    x_pos = list(subset_df["syllable"].unique()).index(syllable)
                    ax.scatter(x_pos, -0.1, color="red", marker="*", s=100)
        
        plt.xlabel("Syllables sorted by frequency")
        plt.ylabel(stat)
        plt.title(f"Syllable Frequency on {group_plot} = {day}")
        plt.xticks(rotation=90)
        sns.despine()
        
        # Save figure
        plot_name = f"{stat}_syll_freq_{group_plot}_{day}"
        save_analysis_figure(fig, plot_name, project_dir, model_name, save_dir)
...
def plot_indv_syllables(
    stats_df,
    project_dir,
    model_name,
    group_plot="group2",  # Defines x-axis (e.g., days)
    group_diff="group1",  # Defines different conditions (e.g., treatments)
    save_dir=None,
    stat="frequency",
    colors=None,
    figsize=(8, 5),
):
    """Plot syllable frequency trends across `group2` levels, comparing `group1` conditions within each plot.
    One plot per syllable, with `group2` on the x-axis and `group1` levels connected by lines.
    """
    
    if group_plot not in stats_df.columns or group_diff not in stats_df.columns:
        raise KeyError(f"Both '{group_plot}' and '{group_diff}' must be present in stats_df.")
    
    # Sort group_plot labels
    stats_df[group_plot] = pd.Categorical(stats_df[group_plot], ordered=True, categories=sorted(stats_df[group_plot].unique()))
    
    unique_syllables = stats_df["syllable"].unique()
    figs = []
    
    for syllable in unique_syllables:
        subset_df = stats_df[stats_df["syllable"] == syllable]
        
        fig, ax = plt.subplots(figsize=figsize)
        
        # Line plot to show trends across group2 levels
        sns.pointplot(
            data=subset_df,
            x=group_plot,
            y=stat,
        ...
        plt.xlabel(group_plot)
        plt.ylabel(stat)
        plt.title(f"Syllable {syllable} Frequency Across {group_plot} Levels")
        plt.xticks(rotation=45)
        sns.despine()
        
        # Save figure
        plot_name = f"{stat}_syll_trend_{syllable}"
        save_analysis_figure(fig, plot_name, project_dir, model_name, save_dir)
...
def generate_transition_matrices(
    ...
    label_group = list(index_data.group1.values)
    recordings = list(index_data.name.values)
    group = sorted(list(index_data.group1.unique())) 
